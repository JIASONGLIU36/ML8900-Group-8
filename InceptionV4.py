...
def train_models(train_datapath, feature, log_path, pretrained_weights=None):
    ...
    model_name = "{weight_name}_{featureName}".format(weight_name = 'InceptionV4', featureName = feature) + ".keras"
...
def detect_image(input_image, weight_path, feature):
...
if __name__ == '__main__':
    ...
    # Total_classes = {'hairstyle': 2, 'emotion': 3, 'eyecolor': 9}
    # Get current path
    curr_path = os.getcwd()
    ...
    if args.action == "train":
            if args.weights != None:
                try:
                    ...
                    model_name = train_models(default_datapath, args.feature, args.log, pretrained_weights = args.weights)
                ...
                model_name = train_models(args.source, args.feature, args.log)
                print(args.log + "//"+ model_name)
            else:
                print("loading image from source: "+default_datapath)
                model_name = train_models(default_datapath, args.feature, args.log)
        ...
        if args.weights != None:
            print("Load weights from: "+args.weights)
            detect_image(args.image, args.weights, args.feature)
        else:
