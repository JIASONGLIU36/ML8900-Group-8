...
def train_models(train_datapath, num_of_classes, model_type, log_path, enable_dropout, pretrained_weights=None):
...
def detect_image(input_image, model_type, weight_path, num_of_classes, enable_dropout):
...
# for detection: python VGG.py detect --weights=./logs/1e13f792-edc1-4f8d-be20-7ef99fde40fe_20231108_VGG19.keras --image=./abc.jpg --model VGG19
# for training: python VGG.py train --model VGG19 --dropout
if __name__ == '__main__':
    ...
    if args.action == "train":
            if args.weights != None:
                ...
                model_name = train_models(args.source, Num_classes, args.model, args.log, args.dropout)
                print(args.log + "//"+ model_name)
            else:
                print("loading image from source: "+curr_path+"\\animeFaces")
                model_name = train_models(default_datapath, Num_classes, args.model, args.log, args.dropout)
        ...
        if args.weights != None:
            print("Load weights from: "+args.weights)
            detect_image(args.image, args.model, args.weights, Num_classes, args.dropout)
        else:
