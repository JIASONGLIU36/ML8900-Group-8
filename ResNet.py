...
def train_models(train_datapath, num_of_classes, model_type, log_path, pretrained_weights=None):
    ...
    model_name = "{uuid}_{date:%Y%m%d}_".format(uuid = uuid.uuid4(), date = datetime.now()) + model_type + ".keras"
...
def detect_image(input_image, model_type, weight_path, num_of_classes):
...
# for detection: python Resnet.py detect --image=./abc.jpg --weights=./logs/7c98ff7f-7fa5-4557-840d-3d2952af1790_20231108_resnet34.keras --model=resnet34
# for training: python Resnet.py train --model resnet34 --weights=./logs/b1198048-e1a1-4fe4-8f8d-1953d093b2db_20231108_resnet34.keras
    ...
    if args.action == "train":
            if args.weights != None:
                try:
                    ...
                    model_name = train_models(default_datapath, Num_classes, args.model, args.log, pretrained_weights = args.weights)
                ...
                model_name = train_models(args.source, Num_classes, args.model, args.log)
                print(args.log + "//"+ model_name)
            else:
                print("loading image from source: "+curr_path+"\\animeFaces")
                model_name = train_models(default_datapath, Num_classes, args.model, args.log)
        ...
        if args.weights != None:
            print("Load weights from: "+args.weights)
            detect_image(args.image, args.model, args.weights, Num_classes)
        else:
